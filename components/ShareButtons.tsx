...
export default function ShareButtons({
  ...
  useEffect(() => {
    ...
    document.onscroll = function () {
      ...
      if (
        window.innerHeight + window.scrollY >
        0.03 * document.body.clientHeight
      ) {
        setTop(80);
      } else {
        setTop(350);
  ...
  return (
    <div
      className="w-[250px] mb-4 items-center justify-center p-4"
    //   style={{ position: "fixed", top: top }}
    >
      ...
      {shareLinks.map((link) => (
        <button
          ...
          className={`p-2 mr-2 rounded-full bg-gray-200 text-gray-600 transition-colors ${link.color} hover:text-white`}
